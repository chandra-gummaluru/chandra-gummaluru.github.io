...
> Let $P, Q \subseteq \mathbb{R}^n$ be two convex sets such that $P \cap Q = \emptyset$. Then, there exists an $a \in \mathbb{R}^n$ and $b \in \mathbb{R}$ such that for all $x \in P$, we have $a^{\top}x + b \geq 0$, and for all $x \in Q$, we have $a^{\top}x + b \leq 0$.
...
First, let $x \in Q$. Then for any $t \in [0,1]$, the point $tx + (1-t)q^* \in Q$ and so
 ...
 \[a^{\top}x - (p^*-q^*)^{\top}q^* - \Delta \leq 0, \forall x \in Q,\]
 and
 \[a^{\top}x - (p^*-q^*)^{\top}p^* + \Delta \geq 0, \forall x \in P,\]\]
 where $a = p^* - q^*$. If we can find a $\Delta \geq 0$ for which $-(p^* - q^*)^{\top}q^* - \Delta = -(p^* - q^*)^{\top}p^* + \Delta = b$, the proof is complete. We have
 \[2\Delta = -(p^* - q^*)^{\top}q^* + (p^* - q^*)^{\top}p^*\]
 $\begin{aligned}
 &\Rightarrow 2\Delta = (p^* - q^*)^{\top}(p^* - q^*) \\
 &\Rightarrow \Delta = \frac{\|p^* - q^*\|_2^2}{2}.
 \end{aligned}$
 </p>
</details>
