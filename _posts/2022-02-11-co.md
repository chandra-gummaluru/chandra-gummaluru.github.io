...
> **Def. Derivative**:
> The rate of change of a function, $f: \mathbb{R}^n \rightarrow \mathbb{R}$, at some point $\mathbf{z} \in \text{dom}(f)$ in the direction, $\mathbf{v} \in \mathbb{R}^n, \|v\|\_2 = 1$ is defined as
> \\[D_{\mathbf{v}}[f](x) := \lim_{t \rightarrow 0}\frac{f(\mathbf{x} + t\mathbf{v}) - f(\mathbf{x})}{t}.\\]
> We call $D_{\mathbf{v}}f(\mathbf{x}_0)$ the **directional derivative** of $f$ at $x_0$ in the direction, $v$. If $\mathbf{v} = \mathbf{e}\_1$, then we call $D_{\mathbf{v}}f(\mathbf{x}_0)$ the **partial derivative** of $f$ at $\mathbf{x}_0$ w.r.t. $x_i$, and denote it as $\left.\partial f / \partial x_i\right\vert_{x=x_0}$.
...
<br>
We call $\nabla f(x_0)$ the **gradient** of $f$ at $x_0$. We say $f$ is **differntiable** at $x_0$ if $\nabla f(x_0)$ exists.
...
> Let $f: \mathbb{R}^n \rightarrow \mathbb{R}$. Then, $D_{u}\left(D_vf\right) = u^{\top}\nabla ^2 fv$, i.e., $u^{\top}\nabla ^2fv$ is the directional derivative of the directional derivative of $f$ in the direction $v$, in the direction, $u$.
> 
<details>
 <summary><strong>See proof</strong>.</summary>
<p>
We have
\[u^{\top}\nabla^2 f(x_0)v = \sum_{i=1}^{n}\sum_{j=1}^{n}u_iv_j\frac{\partial^2 f}{\partial x_i \partial x_j}\]
$\begin{aligned}
...
<br>
We call $\nabla^2f(x_0)$ the **Hessian** of $f$ at $x_0$. We say $f$ is **twice-differntiable** at $x_0$ if $\nabla^2f(x_0)$ exists.
