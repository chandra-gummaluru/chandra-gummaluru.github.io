...
<br><br>
The conditions derived thus far are fairly weak. However, we can derive stronger conditions in some cases. To do this, we first the so-called Lagrangian of $f$ as
\\[L(x,\lambda,\mu) = f(x) + \sum_{i=1}^{m}\mu_ig_i(x) + \sum_{j=1}^{n}\lambda_jh_j(x).\\]
If we assume that $\mu_i \geq 0, \forall i$, then for any feasible $x$, we have $L(x,\lambda,\mu) \leq f(x)$, with equality whenever $\mu_i = 0, \forall i$. Hence, we can recover $f$ from $L_f$ by taking the supremum of $L_f$, w.r.t. $\lambda$ and $\mu$ subject to $\mu \geq 0$ i.e.,
\\[f(x) = \sup_{\lambda, \mu \geq 0}L(x,\lambda,\mu).\\]
Now suppose we define 
\\[\zeta(\lambda,\nu) = \inf_{x \in \mathcal{X}}L(x,\lambda,\nu).\\]
Then, we have
\\[\inf_{x \in \mathcal{X}}f(x) = \inf_{x \in \mathcal{X}}\sup_{\lambda, \mu \geq 0}L(x,\lambda,\mu) \geq \sup_{\lambda,\mu\geq 0}\inf_{x \in \mathcal{X}}L(x,\lambda,\mu) = \sup_{\lambda,\mu \geq 0}\zeta(\lambda,\mu).\\]
We refer to $\inf_{x \in \mathcal{X}}f(x)$ as the **primal problem**; the minimizer is denoted $x^\*$ and $f(x^\*)$ is the minimum. We refer to $\sup_{\lambda,\mu \geq 0}\zeta(\lambda,\mu)$ as the **dual problem**; the maximizer is denoted $(\lambda^\*, \mu^\*)$ and $\zeta(\lambda^\*, \mu^\*)$ is the maximum.
<br><br>
The **duality-gap** is $f(x^\*) - \zeta(\lambda^\*, \mu^\*)$, which is always non-negative as per the previous argument. If the duality-gap is zero, i.e., the primal and dual solutions are the same, then
   \\[f(x^\*) = \zeta(\lambda^\*, \mu^\*) = \inf_{x \in \mathcal{X}}L(x,\lambda^\*,\nu^\*),\\]
   which means that $x^\*$ is a local minimizer of $L$, and so $\nabla L(x^\*,\lambda^\*,\mu^*) = 0$.
   However we also have
   \\[\inf_{x \in \mathcal{X}}L(x,\lambda^\*,\nu^\*) = \inf_{x \in \mathcal{X}}\left\lbrace f(x) + \sum_{i=1}^{m}\mu_i^\*g_i(x) + \sum_{j=1}^{n}\lambda_j^\*h_j(x) \right\rbrace =\inf_{x \in \mathcal{X}}\left\lbrace f(x) + \sum_{i=1}^{m}\mu_i^\*g_i(x) \right\rbrace \\]
   which means that $\sum_{i=1}^{m}\mu_i^\*g_i(x^\*) = 0$, and so $\mu_i^\*g_i(x^\*) = 0$.
...
Of course, none of the above conditions are sufficient, i.e., just because they are met at some point $x^\*$ does not mean that $x^\*$ is actually a minimizer. For that, we will need to develop some sufficient conditions.
...
### Operations that Preserve Convexity
   - **Removing Equality Constraints**: Let $g(z) = f(Fx+x_0)$, where $x_0$ is such that $Ax_0 + b = 0$, and $F$ is such that $\text{range}(F) = \text{null}(A)$. Then, \\[A\left(Fz + x_0\right) - b = A(Fz) + Ax_0 - b= 0 + b - b= 0.\\] Therefore, if $z^\*$ is a local minimizer of $g$, then $x^\* = Fz^\* + x_0$ is a local minimizer of $f$.
   \\[\begin{equation}\begin{aligned}\text{minimize } &t \\\\ \text{subject to } &f(x) \leq t \\\\ &g_i(x) \leq 0, i = 1,\dots, m \\\\ &h_j(x) =0, j = 1, \dots, n\end{aligned}\end{equation}\\]
   This program finds the smallest $t$ such that no part of $\text{epi}(f)$ is below $t$.
   
   - **Epigraph Formulation**: Define an auxillary variable, $t \in \mathbb{R}$ and consider the program
   \\[\begin{equation}\begin{aligned}\text{minimize } &t \\\\ \text{subject to } &f(x) \leq t \\\\ &g_i(x) \leq 0, i = 1,\dots, m \\\\ &a^{\top}_jx+b_j =0, j = 1, \dots, n\end{aligned}\end{equation}\\]
   This program finds the smallest $t$ such that no part of $\text{epi}(f)$ is below $t$.
