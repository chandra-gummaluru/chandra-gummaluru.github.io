...
> A set is convex if it contains all convex combination of its points. The convex hull of a (possibly non-convex) set, $S$, denoted $\text{conv}(S)$, is the set of all convex combinations of its points; $\text{conv}(S) \superseteq S$ and for any other convex set $A$, we have $A \superseteq \text{conv}(S)$.

### Examples of Convex Sets
We now consider some examples of convex sets:

> **Def. Linear Set** An **linear combination** of the points $x_1, \dots, x_n, \mathbb{R}$ is of the form
> \\[\sum_{i=1}^{n}\theta_ix_i.\\]
> A set is linear if it contains all linear combinations of its points. The linear hull of a (possibly non-linear) set, $S$, denoted $\text{lin}(S)$, is the set of all linear combinations of its points; $\text{lin}(S) \superseteq S$ and for any other linear set $A$, we have $A \superseteq \text{lin}(S)$.

> **Def. Affine Set** An **affine combination** of the points $x_1, \dots, x_n, \mathbb{R}$ is of the form
> \\[\sum_{i=1}^{n}\theta_ix_i, \sum_{i=1}^{n}\theta_i.\\]
> A set is affine if it contains all affine combinations of its points. The affine hull of a (possibly non-affine) set, $S$, denoted $\text{aff}(S)$, is the set of all affine combinations of its points; $\text{aff}(S) \superseteq S$ and for any other affine set $A$, we have $A \superseteq \text{aff}(S)$.

> **Def. Conic Set** A **conic combination** of the points $x_1, \dots, x_n, \mathbb{R}$ is of the form
> \\[\sum_{i=1}^{n}\theta_ix_i, \theta_i \geq 0.\\]
> A set is conic if it contains all conic combinations of its points. The conic hull of a (possibly non-conic) set, $S$, denoted $\text{cone}(S)$, is the set of all conic combinations of its points; $\text{cone}(S) \superseteq S$ and for any other conic set $A$, we have $A \superseteq \text{cone}(S)$.
