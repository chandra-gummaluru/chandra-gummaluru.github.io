...
> **Thm. Chain Rule**:
> Let $f: \mathbb{R}^n \rightarrow \mathbb{R}$, $g: \mathbb{R} \rightarrow \mathbb{R}^n$ and $x = g(t)$. Then,
> \\[\frac{df}{dt} = \sum_{i=1}^{n}\frac{\partial f}{\partial x_i}\frac{dg_i}{dt}.\\]

It turns out that we can express $D_vf$ in terms of $\nabla f$.
...
<p>
Define the function $g(t) = x + tv$ and $h = f \circ g$. It follows that
 \\[D_vf(x) = \frac{dh}{dt}(0).\\]
Using the chain-rule, we compute
\\[\frac{\partial h}{\partial t} = \sum_{i=1}^{n}\frac{\partial h}{\partial g_i}\frac{dg_i}{dt}\\]
$\begin{aligned}
&= \sum_{i=1}^{n}\frac{\partial h}{\partial g_i}v_i \\
&= \sum_{i=1}^{n}\frac{\partial f}{\partial x_i}v_i
\end{aligned}$
 $\blacksquare$
</p>
</details>
