...
\[\mathcal{D'} = \lbrace (t,v,w) \text{ s.t. } \exists x \in \mathcal{D}, f(x) = t, g(x) = v, Ax-b= w \rbrace.\]
We can then define $p^\* = \inf_{(t,v,w) \in \mathcal{D'}}t$ and express the dual function as
\\[\zeta(\lambda, \mu) = \inf_{(t,v,w) \in \mathcal{D'}}\left\lbrace t + \lambda^{\top}v + \mu^{\top}w \right\rbrace\\]
...
represents a hyper-plane normal to $\begin{bmatrix} \lambda^{\top} & \mu^{\top} & 1 \end{bmatrix}^{\top}$ and whose $t$-intercept is $k$. Moreover, $\zeta(\lambda,\mu)$ is the smallest $t$-intercept of those hyper-planes that intersect $\mathcal{S}$. Thus, the dual optimal, $d^\*$, is the largest $t$-intercept of all hyper-planes that intersect $\mathcal{S}$ in such a way that shifting any hyper-plane along its normal seperates it from $\mathcal{S}$. We also see that since 
\\[f(x^\*) = \begin{bmatrix} \lambda^{\top} & \mu^{\top} & 1 \end{bmatrix}\begin{bmatrix} 0 \\\\\\ 0 \\\\\\ f(x^\*) \end{bmatrix},\\]
and so $f(x^\*) \geq \zeta(\lambda, \mu)$.
