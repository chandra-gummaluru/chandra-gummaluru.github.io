...
> Given a feasible point, $x \in \mathcal{X}$ for (OptPrb), a direction, $v \in \mathbb{R}^n, \\|v\\|\_2 = 1$ is **feasible** if there exists an $T > 0$ for which $x + tv \in \mathcal{X}$ for all $0 \leq t \leq T$.
