...
In the following, we will consider multi-variable scalar functions, i.e., those of the form, $f: \mathbb{R}^n \rightarrow \mathbb{R}$. We will use the notation, $\text{dom}(f)$ and $\text{cdm}(f)$ to respectively denote the domain and co-domains of $f$. Note that $\text{dom}(f) \subseteq \mathbb{R}^n$ and $\text{cdm}(f) \subseteq \mathbb{R}$.

> **Def. Derivative**:
> The rate of change of a function, $f: \mathbb{R}^n \rightarrow \mathbb{R}$, at some point $x_0 \in \text{dom}(f)$ in the direction, $v \in \mathbb{R}^n$ is defined as
> \\[D_vf(x_0) := \lim_{t \rightarrow 0}\frac{f(x_0 + tv) - f(x_0)}{t}.\\]
> We call $D_vf(x_0)$ the **directional derivative** of $f$ at $x_0$ in the direction, $v$. If $v = e_i$, then we call $D_{v}f(x_0)$ the **partial derivative** of $f$ at $x_0$ w.r.t. $x_i$, and denote it as $\left.\partial f / \partial x_i\right\vert_{x=x_0}$.
...
> \\[D_vf(x_0) = \nabla f^{\top}(x_0)v.\\]
> where
> \\[\nabla f(x_0)\_i := \left.\frac{\partial f}{\partial x_i}\right\vert_{x=x_0}.\\]

<details>
 <summary><strong>See proof</strong>.</summary>
<p>
Define the function $g(t) = x_0 + tv$. Then,
\[D_vf(x_0) = \lim_{t \rightarrow 0}\frac{f(x_0+tv) - f(x_0)}{t}\]
$\begin{aligned}
...
&= \sum_{i=1}^{n}\left.\frac{\partial f}{\partial x_i}\right\vert_{x=x_0}v_i \\
&= \nabla f^{\top}(x_0)v
\end{aligned}$
<br>
$\blacksquare$
...
<br>
We call $\nabla f(x_0)$ the **gradient** of $f$ at $x_0$. We say that $f$ is **differntiable** at $x_0$ if $\nabla f(x_0)$ exists.
...
We can use the aforementioned definitions to compute higher-order derivatives of a scalar function, $f: \mathbb{R}^n \rightarrow \mathbb{R}$. For example, the 2$\textsuperscript{nd}$ derivative of $f$ is the gradient of the gradient of $f$, i.e.,
\\[\nabla^2 f = \nabla\left(\nabla f\right) = \begin{bmatrix} \frac{\partial^2 f_1}{\partial x_1^2} & \dots & \frac{\partial^2 f_1}{\partial x_1x_n} \rightarrow \vdots & \ddots & \vdots & \frac{\partial^2 f_m}{\partial x_nx_1} & \dots & \frac{\partial^2 f_m}{\partial x_n^2}\end{bmatrix}.\\]
