...
It turns out that
\\[P(x) = \sup_{\lambda \geq 0}\left\lbrace \sum_{i=1}^{m}\lambda_ig_i(x) + \sum_{j=1}^{n}\mu_jh_j(x)\right\rbrace,\\]
where $\lambda \in \mathbb{R}^m$ and $\mu \in \mathbb{R}^{n}\_{+}$.
To see this, we consider two cases:
 
 - if $x \in \mathcal{X}$, we have \\[P(x) = \sup_{u \geq 0}\left\lbrace \sum_{i=1}^{m}\lambda_ig_i(x) \right\rbrace,\\] where $g_i(x) \leq 0$. Thus, by choosing $\lambda_i = 0, \forall i$, we get $P(x) = 0$.
...
Then, we have
\\[\inf_{x \in \mathcal{X}}f(x) = \inf_{x}\bar{f}(x) = \inf_{x}\sup_{\lambda \geq 0, \mu}L(x,\lambda,\mu) \geq \sup_{\lambda\geq 0, \mu}\inf_{x}L(x,\lambda,\mu) = \sup_{\lambda \geq 0, \mu}\zeta(\lambda,\mu).\\]
