...
The **duality-gap** is $f(x^\*) - \zeta(\lambda^\*, \mu^\*)$, which is always non-negative as per the previous argument. If the duality-gap is zero, i.e., the primal and dual solutions are the same, then
   ...
   \\[\inf_{x \in \mathcal{X}}L(x,\lambda^\*,\nu^\*) = \inf_{x \in \mathcal{X}}\left\lbrace f(x) + \sum_{i=1}^{m}\mu_i^\*g_i(x) + \sum_{j=1}^{n}\lambda_j^\*h_j(x)\inf_{x \in \mathcal{X}}\left\lbrace f(x) + \sum_{i=1}^{m}\mu_i^\*g_i(x) \\]
