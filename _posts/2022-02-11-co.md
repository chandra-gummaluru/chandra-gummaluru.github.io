...
> Given a feasible point, $x \in \mathcal{X}$ for (OptPrb), a direction, $v \in \mathbb{R}^n, \|v\|_1$ is **feasible** if there exists an $t > 0$ for which $x + tv \in \mathcal{X}$.
...
To solve (OptPrb), it will be useful to introduce a few definitions.
<br><br>
As mentioned previously, we consider optimizing (minimizing) multi-variable scalar functions, i.e., those of the form, $f: \mathbb{R}^n \rightarrow \mathbb{R}$, where $x \mapsto f(x)$. We will use the notation, $\text{dom}(f)$ and $\text{cdm}(f)$ to respectively denote the domain and co-domains of $f$. Note that $\text{dom}(f) \subseteq \mathbb{R}^n$ and $\text{cdm}(f) \subseteq \mathbb{R}$.
...
> Let $f$ and $\mathcal{X}$ be defined as in (OptPrb).
