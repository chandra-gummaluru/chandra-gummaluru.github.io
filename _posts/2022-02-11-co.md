...
### Operations that Preserve Convexity
   - **Removing Equality Constraints**: Suppose $x_0$ is such that $Ax_0 + b = 0$, and $\text{range}(F) = \text{null}(A)$. Then, since $A(Fz) = 0$ for any $z$, we have $A\left(Fz + x_0\right) = A(Fz) + Ax_0 = 0 + b = b$, i.e., $Fz + x_0$ satisfies the linear equality constraint for any $z$.
