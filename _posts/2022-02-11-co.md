...
represents a hyper-plane normal to $\begin{bmatrix} \lambda^{\top} & \mu^{\top} & 1 \end{bmatrix}^{\top}$ and whose $t$-intercept is $k$. Moreover, $\zeta(\lambda,\mu)$ is the smallest $t$-intercept of those hyper-planes that intersect $\mathcal{S}$. Thus, the dual optimal, $z^\*$, is the largest $t$-intercept of all hyper-planes that intersect $\mathcal{S}$ in such a way that shifting any hyper-plane along its normal seperates it from $\mathcal{S}$. We also see that since 
\\[f^* = \begin{bmatrix} \lambda^{\top} & \mu^{\top} & 1 \end{bmatrix}\begin{bmatrix} 0 \\\\\\ 0 \\\\\\ f^\* \end{bmatrix},\\]
and $(0,0,f^\*) \in \mathcal{S}$, it follows that $f^\* \geq \zeta(\lambda, \mu)$.
...
### Duality in Convex Optimization
   ...
   <br><br>
   Thus, by the supporting hyper-plane theorem, we can write
   \\[\begin{bmatrix} \zeta^{\top} & \eta^{\top} & w \end{bmatrix}\begin{bmatrix} v \\\\\\ u \\\\\\ t  \end{bmatrix} \geq \begin{bmatrix} \zeta^{\top} & \eta^{\top} & w \end{bmatrix}\begin{bmatrix} 0 \\\\\\ 0 \\\\\\ f^\* \end{bmatrix} = wf^\*, \forall \begin{bmatrix} v \\\\\\ u \\\\\\ t \end{bmatrix} \in \mathcal{S}.\\]
   Since $\begin{bmatrix} g(x) & h(x) & f(x) \end{bmatrix}^{\top} \in \mathcal{S}, \forall x \in \mathcal{D}$, if $w \neq 0$, we may write
   \\[\lambda^{\top}g(x) + \mu^{\top}h(x) + f(x) \geq f^\*, \forall x \in \mathcal{D},\\]
   where we defined $\lambda = \zeta / w$ and $\mu = \eta / w$.
