...
In other words, there must exist a neighbourhood around $x$ for which all points other than $x$ are infeasible or sub-optimal.
...
<br>
Now consider a feasible point $y$ such that $f_0(y) < f_0(x)$. Then, it must be that $\|x - y\|_2 > R$. If we take $z = \theta x + (1-\theta)y$ where $\theta = R/\left(2\|x-y\|_2\right)$. Since $\|y-x\|_2 > R$, it follows that $0 \leq \theta \leq 1/2$. In other words, $z$ is a convex combination of two feasible points, and is hence, also feasible. Moreover, $\|x-z\|_2 = R/2$.
<br>
Since $f_0$ is convex, it follows that $f_0(z) = f_0\left(\theta x + (1-\theta)y\right) \leq \theta f_0(x) + (1-\theta)f_0(y) < f_0(x)$, where the last inequality follows from the fact that $f_0(y) < f_0(x)$. However, we also see that $\|x-z\|_2 = R/2$. Thus, $x$ is not locally optimal within the $R$-ball centered at $x$. This is a contradiction.
</p>
</details>
...
A **convex optimization** problem in stanadrd form is one in which:
 ...
 - $f_i$ is convex for $i = 1, \dots, m$
 - $h_j$ is affine, i.e., $h_j(x) = a^{\top} _jx + b_j$ for $j = 1, \dots, p$.

Often, we will simply replace the equality constraints with the sole constraint $Ax=b$.
