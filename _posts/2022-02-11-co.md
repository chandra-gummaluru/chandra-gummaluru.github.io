...
It turns out that
\\[P(x) = \sup_{u \geq 0}\left\lbrace \sum_{i=1}^{m}\lambda_ig_i(x) + \sum_{j=1}^{n}\mu_jh_j(x)\right\rbrace,\\]
...
\\[\inf_{x}\sup_{\lambda,\mu\geq 0}\left\lbrace f(x) + \sum_{i=1}^{m}\lambda_ig_i(x) + \sum_{j=1}^{n}\mu_jh_j(x) \right\rbrace.\\]
