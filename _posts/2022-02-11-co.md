...
There are several operations that preserve convexity. Such operations include, but are not limited to:
 ...
 - **Non-Negative Weighted Sum:** If $f_1,\dots,f_n: \mathbb{R}^n \rightarrow \mathbb{R}$ are convex, then $g = \sum_{i=1}^{n}w_if_i$, where $w_i \geq 0$ is also convex.
 ...
 - **Pointwise Maximum**: If $f_1, \dots, f_n: \mathbb{R}^n \rightarrow \mathbb{R}$ are convex, then $g: \mathbb{R}^n \rightarrow \mathbb{R}$ defined point-wise so that $g(x) = \max\left\lbrace f_1(x), \dots, f_n(x) \right\rbrace$.

 - **Pointwise Supremum**: If $f: \mathbb{R}^{n} \rightarrow \mathbb{R}$ is convex$, then $g: \mathbb{R}^{n-1} \rightarrow \mathbb{R}$, defined point-wise so that
 - \[g(x_1,\dots,x_{i-1},x_{i+1},\dots,x_n) = \sup_{x_i}f(x_1,\dots,x_n),\]
 - is also convex.
