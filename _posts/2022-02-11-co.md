...
<summary><strong>See proof</strong>.</summary>
<p>
Since $\mathcal{D}$ is the intersection of convex sets and is hence also convex. Thus, if $x, y \in \mathcal{D}$, then $z = \theta y + (1-\theta)x \in \mathcal{D}$, where $\theta \in [0,1]$. If $x, y \in \mathcal{X}$, then by definition, $f_i(x), f_i(y) \leq 0, i = 1, \dots, m$ and $h_j(x), h_j(y) = 0, j = 1, \dots, p$
<br><br> 
