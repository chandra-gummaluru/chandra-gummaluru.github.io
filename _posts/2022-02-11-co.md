...
It turns out that
\\[P(x) = \sup_{u \geq 0}\sum_{i=1}^{m}\mu_ig_i(x) + \sum_{j=1}^{n}\lambda_jh_j(x).\\]
To see this, we consider two cases:
- if $x \in \mathcal{X}$, we have
- \\[P(x) = \sup_{u \geq 0}\sum_{i=1}^{m}\mu_ig_i(x),\\]
- where $g_i(x) \leq 0$. So by choosing $\mu_i = 0, \forall i$, we get $P(x) = 0$.

- if$x \not\in \mathcal{X}$, then either $h_j(x) \neq 0$ for some $j$, in which case we can choose $\lambda_j$ to make $\lambda_j h_j(x)$ arbitrarily large, or $g_i(x) > 0$ for some $i$, in which case we choose $\mu_j \geq 0$ to make $\mu_jg_j(x)$ arbitrarily large.
