...
> **Def. Derivative**:
> Let $v \in \mathbb{R}^n, \|v\|\_2 = 1$. The $v$-**semi-directional derivative** of a function, $f: \mathbb{R}^n \rightarrow \mathbb{R}$ is a function, denoted $D_{v+}f: \mathbb{R}^n \rightarrow \mathbb{R}$, where $D_vf(z)$ gives the rate of change of $f$ at $z$ in the direction $v$, and is defined point-wise as
> \\[D_{v+}f(z) := \lim_{t \rightarrow 0^+}\frac{f(z + tv) - f(z)}{t}.\\]
> If $v = e^{(i)}$, we call $D_{v}f$ the **semi-partial derivative** of $f$ w.r.t. $x_i$, and denote it as $\partial f / \partial x_i^+$.
