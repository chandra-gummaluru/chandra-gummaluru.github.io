...
\\[S = \lbrace (t,v,w) \text{ s.t. } f(x) \leq t, g(x) \leq v, Ax-b = w \rbrace.\\]
We can now frame the original problem in terms of $S$:
...
is a hyper-plane normal to $\begin{bmatrix} \lambda^{\top} & \mu^{\top} & 1 \end{bmatrix}^{\top}$ and whose $t$-intercept is $k$. Moreover, $\zeta(\lambda,\mu)$ is the smallest $t$-intercept of those hyper-planes that intersect $\mathcal{S}$.
<br><br>
Likewise, the $t$-intercept of $\mathcal{S}$, 
Thus, we have strong duality if this $t$-intercept is equal to the $t$-intercept of $\mathcal{A}$.
