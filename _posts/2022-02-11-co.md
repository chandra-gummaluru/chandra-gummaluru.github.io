...
> Let $P, Q \subseteq \mathbb{R}^n$ be two convex sets such that $P \cap Q = \emptyset$. Then, there exists a $c \in \mathbb{R}^n$ and $b \in \mathbb{R}$ such that for all $x \in P$, we have $c^{\top}x + b < 0$, and for all $x \in Q$, we have $c^{\top}x + b > 0$.
...
First, let $x \in Q$. Then for any $t \in [0,1]$, the point $tx + (1-t)q^* \in Q$ and so
 ...
 \end{aligned}$
 <br><br>
 If we can find a $\Delta \geq 0$ for which $-(p^* - q^*)^{\top}q^* - \Delta = -(p^* - q^*)^{\top}p^* + \Delta$, the proof is complete. We have
 \[2\Delta = -(p^* - q^*)^{\top}q^* - (p^* - q^*)^{\top}p^*\]
 $\begin{aligned}
 &\Rightarrow 2\Delta =(p^* - q^*)^{\top}(-q^*-p^*) \\
 &\Rightarrow \Delta = -\frac{(p^* - q^*)^{\top}(q^*+p^*)}{2}
 \end{aligned}$
 </p>
</details>
