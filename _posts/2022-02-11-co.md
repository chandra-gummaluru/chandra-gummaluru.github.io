...
\\[\zeta(\lambda,\nu) = \inf_{x}L(x,\lambda,\nu).\\]
Then, we have
\\[\inf_{x}bar{f}(x) = \inf_{x}\sup_{\lambda, \mu \geq 0}L(x,\lambda,\mu) \geq \sup_{\lambda,\mu\geq 0}\inf_{x}L(x,\lambda,\mu) = \sup_{\lambda,\mu \geq 0}\zeta(\lambda,\mu).\\]
We refer to $\inf_{x}bar{f}(x)$ as the **primal problem**; the minimizer is denoted $x^\*$ and $f(x^\*)$ is the minimum. We refer to $\sup_{\lambda,\mu \geq 0}\zeta(\lambda,\mu)$ as the **dual problem**; the maximizer is denoted $(\lambda^\*, \mu^\*)$ and $\zeta(\lambda^\*, \mu^\*)$ is the maximum.
<br><br>
