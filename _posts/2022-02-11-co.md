...
The **duality-gap** is $f(x^\*) - \zeta(\lambda^\*, \mu^\*)$, which is always non-negative as per the previous argument. If the duality-gap is zero, i.e., the primal and dual solutions are the same, then
   $f(x^\*) = \zeta(\lambda^\*, \mu^\*) = \inf_{x \in \mathcal{X}}L(x,\lambda^\*,\nu^\*)$; $x^\*$ is a local minimiezer of $L(x,\lambda^\*,\mu^\*)$ and so 
      - $\zeta(\lambda^\*, \mu^\*) = \sup_{\lambda, \mu \geq 0} L(x^\*, \lambda, \mu) = f(x^\*) + \sum_{i=1}^{m}\mu_ig_i(x^\*) + \sum_{j=1}^{n}\lambda_jh_j(x^\*) = f(x^\*)$; $\mu_jg_j(x^\*) = 0, j = 1, \dots, m$.
   
Thus, the following conditions must hold on $(x^\*, \lambda^\*, \mu^\*)$:
   - **primal feasibility: ** $g(x^\*) \leq 0$ and $h(x^\*) = 0$
   - ** dual feasibility: ** $\mu_j \geq 0, j = 1, \dots, m$
   - ** stationariness: **: $\nabla L(x^\*,\lambda^\*, \mu^\*) = 0$
   - ** complimentary slackness: ** $\mu_jg_j(x^\*) = 0, j = 1, \dots, m$
