...
### Duality in Convex Optimization
   ...
   > **Thm. Slater's Condition:** If $\exists x \in \mathcal{D}$, such that $g(x) < 0$, then there exists a non-vertical supporting hyperplane to $\mathcal{S}$ at $(0,0,f^\*)$.
  <details>
 <summary><strong>See proof</strong>.</summary>
<p>
Assume $g(x) < 0$. It follows that $\chi^{\top}g(X) < 0$ since $\chi \geq 0$. If we now assume that $w \neq 0$, we have
\[0 \leq \sum_{i=1}^{m}\chi_ig(x) + \sum_{j=1}^{n}\eta_ih(x) \leq \sum_{j=1}^{n}\eta_ih(x).\]
<br>
$\blacksquare$
</p>
</details>
