...
It is easy to see that $\begin{bmatrix} 0 & 0 & f^\* \end{bmatrix}^{\top} \in \mathcal{S}$. Thus, we can frame the original problem in terms of $\mathcal{S}$:
...
Since every $x \in \mathcal{D}$ corresponds to at least one $\begin{bmatrix} v^{\top} & w^{\top} & t \end{bmatrix} \in \mathcal{S}$, we may write
...
\end{aligned}$
<br><br>
Since $\begin{bmatrix} 0 & 0 & f^\* \end{bmatrix}^{\top} \in \mathcal{S}$ and
...
### Duality in Convex Optimization
   ...
   <br><br>
   Thus, there exists a supporting hyperplane to $\mathcal{S}$ at $\begin{bmatrix} 0 & 0 & f^\* \end{bmatrix}^{\top}$ and we may write
   \\[\begin{bmatrix} \zeta^{\top} & \eta^{\top} & w \end{bmatrix}\begin{bmatrix} v \\\\\\ u \\\\\\ t  \end{bmatrix} \geq \begin{bmatrix} \zeta^{\top} & \eta^{\top} & w \end{bmatrix}\begin{bmatrix} 0 \\\\\\ 0 \\\\\\ f^\* \end{bmatrix} = wf^\*,\\]
   or equivalently,
   \\[\sum_{i=1}^{m}\zeta_iv_i + \sum_{j=1}^{n}\eta_ju_j + wt \geq wf^\*.\\]
   The latter form shows that $\lambda, \mu \geq 0$. Assuming $w \neq 0$, it follows that
   \\[\begin{bmatrix} \zeta^{\top}/w & \eta^{\top}/w & 1 \end{bmatrix}\begin{bmatrix} v \\\\\\ u \\\\\\ t  \end{bmatrix} \geq f^\*.\\]
   We have already seen that the reverse inequality holds if $\eta^{\top}/w \geq 0$. Thus, in that case, we would have equality. If we then let $\lambda = \zeta/w$, $\mu = \eta/w$, and note that $\begin{bmatrix} g(x) & h(x) & f(x) \end{bmatrix}
