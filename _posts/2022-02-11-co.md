...
<br><br>
The **duality-gap** is $f(x^\*) - \zeta(\lambda^\*, \mu^\*)$, which is always non-negative as per the previous argument. If the duality-gap is zero, i.e., the primal and dual solutions are the same, then we have
\\[f(x^\*) = \zeta(\lambda^\*, \mu^\*) = \inf_{x \in \mathcal{X}}L(x,\lambda^\*,\nu^\*)\\]
 $\begin{aligned}
 &= \inf_{x \in \mathcal{X}}\left\lbrace f(x) + \sum_{i=1}^{m}\mu_i^\*g_i(x) + \sum_{j=1}^{n}\lambda_j^\*h_j(x) \right\rbrace
 \end{aligned}$
 <br><br>
 We see that $x\^*$ is a local minimiezer of $L(x,\lambda^*,\mu^*)$. As such, we require that $\nabla L(x^\*,\lambda^*, \mu^\* = 0$.
