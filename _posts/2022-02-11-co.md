...
\\[\zeta(\lambda,\nu) = \underset{x \in \mathcal{X}}{\text{inf}}L(x,\lambda,\nu).\\]
Then, we have
...
<br><br>
The **duality-gap** is $f(x^\*) - \zeta(\lambda^\*, \mu^\*)$, which is always non-negative as per the previous argument. For now, let us assume the duality-gap is zero, i.e., the primal and dual solutions are the same.
