...
\\[S = \lbrace (t,v,w) \text{ s.t. } f(x) \leq t, g(x) \leq v, A(x) = w \rbrace.\\]
We can now frame the original problem in terms of $S$:
\\[\begin{equation}\begin{aligned}\text{minimize } &t \\\\ \text{subject to } &(t,0,0) \in S, \dots, n\end{aligned}\end{equation}\tag{OptPrb}\\]
