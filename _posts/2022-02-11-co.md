...
The **derivative** of $f$ at $x$ in the direction $v$ is given by
\\[D_v f(x) := \lim_{t \rightarrow 0}\frac{f(x + tv) - f(x)}{t}.\\]

Define a function $g(t) = f(x+tv)$. Then, by definition,
\\[\left.\frac{d}{dt}g(t)\right\vert_{t=0} = \lim_{h\rightarrow 0}\frac{g(h) - g(0)}{h}\\]
$\begin{aligned}
&= \lim_{h\rightarrow\infty}\frac{f(x+hv) - f(x)}{h} \\
&= D_v f(x)
\end{aligned}$

However, we also have
\\[\left.\frac{d}{dt}g(t)\right = \frac{\partial f}{\partial t}\\]
...
Finding $x^\*$ is difficult. However, we can derive an lower-bound for $f(x^\*)$. To do this, we first define a function,
\\[L_f(x,\lambda,\mu) = f(x) + \sum_{i=1}^{m}\lambda_ig_i(x) + \sum_{j=1}^{n}\nu_jh_j(x),\\]
where $\lambda_i \geq 0, i = 1, \dots, m$. This definition may seem arbitrary, but the idea is that for any feasible $x$, $L_f(x,\lambda,\mu) \geq f(x)$, and hence, serves as a lower-bound for $f(x^\*)$. To get the best possible lower-bound, we should take the supremum of $L_f$ w.r.t. $\lambda$ and $\mu$.
