...
\\[\begin{aligned}\text{minimize } &f(x) + P(x) \\\\ \text{subject to } &g_i(x) \leq 0, i = 1,\dots, m \\\\ &h_j(x) =0, j = 1, \dots, n\end{aligned},\\]
where
\\[P(x) = \begin{cases}0 &x \in \mathcal{X} \\ \infty & \text{otherwise.}\end{cases}\\]

To do this, we first the so-called Lagrangian of $f$ as
