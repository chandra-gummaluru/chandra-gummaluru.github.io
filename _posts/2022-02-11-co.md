...
> We call $D_vf(x)$ the **derivative** of $f$ at $x$ in the direction, $v$. If $v = e_i$, then we call $D_{v}f$ the **partial derivative** of $f$ at $x$ w.r.t. $x_i$, and denote it as $\partial f / \partial x_i(x)$.
...
<p>
Define the function $g(t) = x + tv$. Then,
\[D_vf(x) = \lim_{t \rightarrow 0}\frac{f(x+tv) - f(x)}{t}\]
$\begin{aligned}
...
\[\left.\frac{\partial h}{\partial t}\vert_{t=0} = \sum_{i=1}^{n}\left.\frac{\partial h}{\partial g_i}\vert_{t=0}\left.\frac{dg_i}{dt}\vert_{t=0}\]
$\begin{aligned}
&= \sum_{i=1}^{n}\left.\frac{\partial h}{\partial g_i}\vert_{t=0}v_i \\
&= \sum_{i=1}^{n}\left.\frac{\partial f}{\partial x_i}\vert_{x}v_i \\
&= \nabla f^{\top}(x)v
\end{aligned}$
 $\blacksquare$
</p>
