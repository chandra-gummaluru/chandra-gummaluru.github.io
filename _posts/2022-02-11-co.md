...
### Duality in Convex Optimization
   ...
   \\[\begin{bmatrix} \lambda^{\top} & \mu^{\top} & w \end{bmatrix}\begin{bmatrix} v \\\\\\ u \\\\\\ t  \end{bmatrix} \geq \begin{bmatrix} \lambda^{\top} & \mu^{\top} & w \end{bmatrix}\begin{bmatrix} 0 \\\\\\ 0 \\\\\\ f^\* \end{bmatrix} = wf^\*, \forall (v, u, t) \in \mathcal{S}.\\]
   Since $(0, 0, f(x)) \in \mathcal{S}, \forall $x \in \mathcal{D}$, if $w \neq 0$, we may write
   \\[\lambda^{\top}g(x) + \mu^{\top}h(x) + f(x) \geq f^\*\\]
