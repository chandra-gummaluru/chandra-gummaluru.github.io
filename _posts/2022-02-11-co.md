...
Neither of the above conditions are sufficient, i.e., just because they are met at some point $x^\*$ does not mean that $x^\*$ is actually a minimizer. For that, we will need to develop some sufficient conditions.
...
Finding $x^\*$ is difficult in general. However, we can derive conditions that $x^\*$ must satisfy. To do this, we first the so-called Lagrangian of $f$ as
