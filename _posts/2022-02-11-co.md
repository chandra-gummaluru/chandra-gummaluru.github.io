...
Finding $x^\*$ in non-convex optimization problems is considerably more difficult. However, we can derive a lower-bound for $f(x^\*)$. To do this, we first the so-called Lagrangian of $f$ as
...
\\[\inf_{x \in \mathcal{X}}f(x) = \inf_{x \in \mathcal{X}}\sup_{\lambda, \mu \geq 0}L(x,\lambda,\mu) \geq \sup_{\lambda,\mu\geq 0}\inf_{x \in \mathcal{X}}L(x,\lambda,\mu) = \sup_{\lambda,\mu \geq 0}\zeta(\lambda,\mu).\\]
 These definitions may seem arbitrary, but the idea is that for any feasible $x$, $L_f(x,\lambda,\mu) \leq f(x)$, and so $\zeta_f(\lambda,\nu) \leq f(x^\*)$.
<br><br>   
Obviously, we want to choose $\lambda$ and $\nu$ so that $\zeta_f(\lambda,\nu)$ is as large as possible. This can be done by solving the maximization problem:
\\[\begin{aligned}\text{maximize } &\zeta_f(\lambda,\mu) \\\\ \text{subject to } &\lambda_i \geq 0, i = 1,\dots, m \end{aligned}\tag{DualOptPrb}\\]
