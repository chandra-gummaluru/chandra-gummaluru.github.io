...
\\[\begin{equation}\begin{aligned}\text{minimize } &f_0(x) \\\\ \text{subject to } &f_i(x) \leq 0, i = 1,\dots, n \\\\ &h_j(x) =0, j = 1, \dots, p\end{aligned}\end{equation}\\]
where
...
\\[\mathcal{D} := \text{dom}(f) \cap \bigcap {i=1}^{m}\text{dom}(g_i) \cap \bigcap_{j=1}^{n}\text{dom}(h_i).\\]
...
\\[f(x) = \inf\big\lbrace f(x) \text{ s. t. } g_i(x) \leq 0, i = 1, \dots, m, h_j(x)  = 0, j = 1, \dots, n \big\rbrace.\\]
