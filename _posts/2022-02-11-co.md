...
In the following, we will consider multi-variable scalar functions, i.e., those of the form, $f: \mathbb{R}^n \rightarrow \mathbb{R}$, where $x \mapsto f(x)$. We will use the notation, $\text{dom}(f)$ and $\text{cdm}(f)$ to respectively denote the domain and co-domains of $f$. Note that $\text{dom}(f) \subseteq \mathbb{R}^n$ and $\text{cdm}(f) \subseteq \mathbb{R}$.
