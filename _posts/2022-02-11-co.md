...
### Operations that Preserve Convexity
   ...
   - **Removing Inequality Constraints**: Let $s \in \mathbb{R}$ be a variable. The constraint, $g(x) \leq 0$ is equivalent to the constraints $g(x) + s = 0$ and $s \geq 0$. Thus, we can define an equivalent program:
   \\[\begin{equation}\begin{aligned}\text{minimize } &f\left(Fz + x_0\right) \\\\ \text{subject to } &g_i\left(Fz+x_0\right) + s_i = 0, i = 1,\dots,m\end{aligned}\end{equation}\\]
