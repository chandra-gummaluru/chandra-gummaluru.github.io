...
> where $h: \mathbb{R} \rightarrow \mathbb{R}^{n \times p}$.
...
It turns out that exponential signals, i.e., those of the form, $e^{st}$, where $s \in \mathbb{C}$, are eigensignals of LTI systems.
...
where $H(s) = \mathscr{L}\lbrace h(t) \rbrace(s)$ is called the **system function**.

<details>
 <summary><strong>See proof</strong>.</summary>
<p>
Let $x(t) = e^{st}$. We have
\[y(t) = \mathcal{T}\lbrace x(t) \rbrace(t) = (h * x)(t)\]
$\begin{aligned}
&= \int_{-\infty}^{\infty}h(\tau)x(t-\tau)d\tau \\
&= \int_{-\infty}^{\infty}h(\tau)e^{s(t-\tau)}d\tau \\
&= e^{st}\underbrace{\int_{-\infty}^{\infty}h(\tau)e^{-s\tau}d\tau}_{\mathscr{L}\lbrace h \rbrace(s)}.
\end{aligned}$
</p>
</details>
