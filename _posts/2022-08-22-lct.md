...
# Systems
> **Def. Continuous-Time System:** A **continuous-time system** is an operator, $\mathcal{T}$, that maps one continuous-time signal, $u: \mathbb{R} \rightarrow \mathbb{R}^m$ to another continuous-time signal, $y = \mathcal{T} \lbrace u \rbrace$, where $y \in \mathbb{R}^p$.
