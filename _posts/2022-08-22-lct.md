...
&= \int_{-\infty}^{-1}h(\tau)u(t - \tau)d\tau + \int_{0}^{\infty}h(\tau)u(t-\tau)d\tau
\end{aligned}$
<br><br>
In order to satisfy causality, the first sum must evaluate to zero for any $x$, which must mean that $h(t) = 0, \forall t < 0$.
<br>
$\blacksquare$
</p>
</details>
