...
Our goal now is to find a closed form expression for $x$ that satisfies (\ref{}) given some initial state $x(0) = x_0 \in \mathbb{R}^{n}$ and input $u$. For now, we will assume that either $B = \bm{0}$ and/or $u \equiv 0$, leading to the following initial-value problem:
\\[\begin{aligned}
\dot{x} &= Ax \\\\\\
x(0) &= x_0.
\end{aligned}\\]
...
Later, we will show that $x(t) = e^{At}x_0$ is indeed a solution to (\ref{}). However, for now, we focus on showing that the matrix series in Def. \ref{} actually converges.
