...
Finding the equilibria of a linear system is relatively straight forward. Indeed, for any given $\bar{u}$, we seek the $x_0$ such that $Ax_0 = B\bar{u}$. If $x_0^\*$ is one such solution, then the set of all solutions is given by $x_0 + \text{ker}(A)$, where $\text{ker}(A)$ denotes the null-space of $A$.
<br><br>
Determining $x(t), \forall t$ now amounts to solving the initial-value problem
\begin{equation}
\begin{aligned}
\dot{x} &= Ax + Bu \\
x(0) &= x_0,
\end{aligned}
\end{equation}
where we assume $u$ is fixed. We will study the solution to (\ref{}) later.

Linear time-invariant (LTI) systems are particularly simple to study because their behaviour can be entirely characterized by their responses to a unit impulse, i.e., their impulse response.
