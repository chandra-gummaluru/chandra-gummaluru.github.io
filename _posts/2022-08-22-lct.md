...
> **Def. Continuous-Time System:** A **continuous-time system** is an operator, $\mathcal{T}$, that maps one continuous-time signal, $u: \mathbb{R}\_{+} \rightarrow \mathbb{R}^m$ to another continuous-time signal, $y = \mathcal{T} \lbrace u \rbrace$, where $y: \mathbb{R}\_{+} \in \mathbb{R}^p$.

We are interested in so-called _causal_ continuous-time systems.
...
We can model the output of a causal system as follows:
...
y &= g(x, u),
\end{aligned}\\]
where $x:\mathbb{R}\_{+} \rightarrow \mathbb{R}^n$, $x(t)$ is the state of the system at time $t$, f: \mathbb{R}^{n} \times \mathbb{R}^{m}$ encodes the state dynamics, and $g: \mathbb{R}^{n} \times \mathbb{R}^{m} \rightarrow \mathbb{R}^{p}$ encodes the output.
<br><br>
In general, $f$ and $g$ vary with time. However, we will only consider systems in which they do not, i.e., time-invariant systems.
<br><br>
