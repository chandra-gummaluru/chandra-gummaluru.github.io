...
We assume that the output of the system at time $t$, i.e., $y(t)$, given some input signal $u$, depends only on the value(s) of $u$ at or before $t$, i.e., $u(0),\dots,u(t)$. Such a system is said to be **causal**. We can model the output of a causal system as follows:
...
<br><br>
We will consider systems in which $f$ and $g$ are constant w.r.t. $t$, or equivalently, $y(t - t_0) = \mathcal{T}\lbrace u(\tau-t_0) \rbrace(t), \forall t_0$, if $y(t) = \mathcal{T}\lbrace u(\tau) \rbrace(t)$. Such systems are called **time-invariant**.
<br><br>
