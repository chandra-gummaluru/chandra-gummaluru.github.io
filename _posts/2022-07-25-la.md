...
> **Def. Linear Combination, Span, and Dimension**:
> Let $\mathcal{X}$ be a linear space over a field, $\mathbb{F}$. A \textbf{linear combination} of $x_1, \dots, x_m \in \mathcal{X}$ is of the form
> \[\lambda_1x_1 + \dots + \lambda_mx_m, \lambda_i \in \mathbb{F}.\]
> The \textbf{span} of $x_1, \dots, x_m$, denoted $\Span(x_1, \dots, x_m)$ is the set of all possible linear combinations of $x_1, \dots, x_m$, i.e.,
> \[\Span(x_1, \dots, x_m) = \left\lbrace \sum_{i=1}^{m}\lambda_ix_i | \lambda_i \in \mathbb{F} \right\rbrace.\]
