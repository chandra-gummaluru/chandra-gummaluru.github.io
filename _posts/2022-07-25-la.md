...
\\[\mathbf{L}(x) = \underbrace{\begin{bmatrix}
t_{1,1} & \dots & t_{1,m} \\\\ \vdots & \ddots & \vdots \\\\ t_{n,1} & \dots & t_{n,m}
\end{bmatrix}}_{L_{\mathscr{X},\mathscr{Y}}}\begin{bmatrix}
x_1 \\\\ \vdots \\\\ x_m
\end{bmatrix}.\\]
...
If $\mathbf{L}$ is an isomorphism, and $L_{\mathscr{X}}$ is its matrix representation under $\mathscr{X}$, and we seek its representation under $\mathscr{A}$, i.e., $L_{\mathscr{A}}$, we have
\\[L_{\mathscr{A}} = T_{\mathscr{A},\mathscr{X}}L_{\mathscr{X},\mathscr{X}}T_{\mathscr{A},\mathscr{X}}.\\]
