...
> -  \textbf{non-negativity}: $\|x\| \geq 0$ for all $x \in \mathcal{X}$ and $\|x\| = 0 \Rightarrow x = \bm{0}$.
...
> These properties imply that $\langle x, x \rangle = 0$ if and only if $x = \bm{0}$.

> **Def. Orthogonality**: Let $\mathcal{X}$ be an inner-product space. Two elements, $x, y \in \mathcal{X}$ are \textbf{orthogonal} iff $\langle x, y \rangle = \bm{0}$.
...
> \begin{proof}
> Assume $x^{(1)}, \dots, x^{(n)}$ are mutually orthogonal, i.e., $\langle x^{(i)}, x^{(j)} \rangle = \bm{0}$ for all $i \neq j$, but that they are not linearly independent, i.e.,
> \\[\sum_{i=1}^{n}\lambda_ix^{(i)} = \bm{0}\\]
> and $\lambda_k \neq 0$ for some $k$. Taking the inner-product of both sides of the above equation with $x^{(j)}$, we have
> \\[\left\langle x^{(k)}, \sum_{i=1}^{n}\lambda_ix^{(i)}\right\rangle = \lambda_k\langle x^{(k)}, x^{(k)} \rangle = \langle x^{(k)}, \bm{0} \rangle = 0.\\]
