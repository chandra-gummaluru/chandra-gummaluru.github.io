...
where $\mathbf{e}^{(i)} \in \mathbb{F}^{n}$ is the vector with every element being the additive identity of $\mathbb{F}$, except the $i$\textsuperscript{th}, which is the multiplicative identity of $\mathbb{F}$. In other words, when we write $\begin{bmatrix}
x_1 & \dots & x_n
...
where $\mathbf{e}^{(i,j)} \in \mathbb{F}^{n,m}$ is the matrix with every element being the additive identity of $\mathbb{F}$, except for the element in the $i$\textsuperscript{th} row and $j$\textsuperscript{th} column which is the multiplicative identity of $\mathbb{F}$. In other words, when we write
\\[A = \begin{bmatrix}
a_{1,1} & \dots & a_{m,1} \\\\
\vdots & \ddots & \vdots \\\\
a_{n,1} & \dots & a_{m,n}
