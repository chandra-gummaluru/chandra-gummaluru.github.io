...
> as the $\lambda$-**eigen-space** of $\mathbf{L}$. Since $\mathbf{L}$ is linear, $x \in \textrm{eig}(\mathbf{L}, \lambda) \Leftrightarrow kx \in \textrm{eig}(\mathbf{L}, \lambda), \forall k \in \mathbf{F}$. The **geometric multiplicity** of $\lambda$ is the dimension of the $\lambda$-eigen-space, i.e., $\dim{\textrm{eig}(\mathbf{L}, \lambda)}$.
