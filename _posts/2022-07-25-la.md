...
\\[\left\lbrace \bm{e}^{(1)}, \dots, \bm{e}^{(n)} \right\rbrace,\\]
where $\bm{e}^{(i)} \in \mathbb{F}^{n}$ is the vector with every element being the additive identity of $\mathbb{F}$, except the $i$\textsuperscript{th}, which is the multiplicative identity of $\mathbb{F}$. In other words, when we write $\begin{bmatrix}
x_1 & \dots & x_n
\end{bmatrix}^{\top}$, we mean that $x = \sum_{i=1}^{n}x_i\bm{e}^{(i)}$. However, it is also possible to represent it using a different basis.
