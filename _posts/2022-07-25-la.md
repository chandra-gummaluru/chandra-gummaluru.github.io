...
Suppose all elements in $\mathcal{X}$ are expressed using the basis $\mathscr{X} = \left\lbrace x^{(1)}, \dots, x^{(m)} \right\rbrace$ and $x = \sum_{i = 1}^{m}x_ix^{(i)}, x \in \mathcal{X}$. Our goal is to find an expression for $L$. To that end, consider applying $L$ on $x$, i.e., $L(x)$. We have
